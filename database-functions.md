...
```sql
CREATE OR REPLACE FUNCTION public.create_booking_with_availability_check(service_id_param uuid, scheduled_date_param date, scheduled_time_param time without time zone, scheduled_datetime_param timestamp with time zone, customer_metrics_id_param uuid, customer_data_param jsonb, project_description_param text, assignment_strategy_param text DEFAULT 'optimal'::text, lead_score_param smallint DEFAULT 0)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
